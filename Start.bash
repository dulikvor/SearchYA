...
initiateAll()
{
	gnome-terminal --tab -e "sh -c \"./ClusterManager/bin/ClusterManager -workingdir= \"" --tab -e "sh -c \"./Third_Party/sbin/mesos-master --ip=$MASTER_ADDRESS --work_dir=$WORKING_AREA_DIR\"" --tab -e "sh -c \"./Third_Party/sbin/mesos-agent --master=$MASTER_ADDRESS:5050 --work_dir=$WORKING_AREA_DIR \"" #--tab -e "sh -c \"./Third_Party/bin/redis-server --loadmodule ./RedisSearchModule/bin/libSearchModule.so -workingdir=../../RedisSearchModule/bin\"" --tab -e "sh -c \"grpcc -p ./Communication/IDL/TextualSearchService.proto -a $MASTER_ADDRESS:50051 -i\""
}
